...
namespace Coprocessor {
    ...
    CP3Config::CP3Config(const std::string& presetOptions) // add new options here!
        ...
        , opt_inprocessInt(_cat, "cp3_inp_cons", "Perform Inprocessing after at least X conflicts", 20000, IntRange(0, INT32_MAX), optionListPtr,
                           &opt_inprocess)
        , opt_inpIntInc(_cat, "cp3_iinp_cons", "Increase inprocessing interval in each iteration", 0, IntRange(0, INT32_MAX), optionListPtr,
                        &opt_inprocess)
        ...
        opt_exit_pp(_cat, "cp3-exit-pp", "terminate after preprocessing (1=exit,2=print formula cerr+exit 3=cout+exit) #NoAutoT", 0, IntRange(0, 3),
                    optionListPtr, &opt_enabled)
        ...
        opt_threads(_cat, "cp3_threads", "Number of extra threads that should be used for preprocessing #NoAutoT", 0, IntRange(0, 64), optionListPtr,
                    &opt_enabled)
        ...
        , opt_check(_cat, "cp3-check", "check solver state during simplification and before returning control to solver #NoAutoT", 0, IntRange(0, 4),
                    optionListPtr, &opt_enabled)
        ...
        opt_bve_limit(_cat_bve, "cp3_bve_limit", "perform at most this many clause derefferences", 25000000, IntRange(-1, INT32_MAX), optionListPtr,
                      &opt_bve)
        , opt_learnt_growth(_cat_bve, "cp3_bve_learnt_growth", "Keep C (x) D, where C or D is learnt, if |C (x) D| <= max(|C|,|D|) + N", 0,
                            IntRange(-1, INT32_MAX), optionListPtr, &opt_bve)
        , opt_resolve_learnts(_cat_bve, "cp3_bve_resolve_learnts",
                              "Resolve learnt clauses: 0: off, 1: original with learnts, 2: 1 and learnts with learnts", 0, IntRange(0, 2),
                              optionListPtr, &opt_bve)
        , opt_unlimited_bve(_cat_bve, "bve_unlimited", "perform bve test for Var v, if there are more than 10 + 10 or 15 + 5 Clauses containing v",
                            false, optionListPtr, &opt_bve)
        ...
        opt_bve_heap(_cat_bve, "cp3_bve_heap",
                     "0: minimum heap, 1: maximum heap, 2: random, 3: ratio pos/neg smaller+less, 4: ratio pos/neg smaller+greater, 5: ratio pos/neg "
                     "greater+less, 6: ratio pos/neg greater + greater, 7-10: same as 3-6, but inverse measure order",
                     0, IntRange(0, 10), optionListPtr, &opt_bve)
        ,
        ...
        , heap_updates(_cat_bve, "bve_heap_updates",
                       "Always update variable heap if clauses / literals are added or removed, 2 add variables, if not in heap", 1, IntRange(0, 2),
                       optionListPtr, &opt_bve)
        ...
        par_bve_threshold(_cat_bve, "par_bve_th", "Threshold for use of BVE-Worker #NoAutoT", 10000, IntRange(0, INT32_MAX), optionListPtr,
                          &opt_par_bve)
        , postpone_locked_neighbors(_cat_bve, "postp_lockd_neighb", "Postpone Elimination-Check if more neighbors are locked #NoAutoT", 3,
                                    IntRange(0, INT32_MAX), optionListPtr, &opt_par_bve)
        , opt_minimal_updates(_cat_bve, "par_bve_min_upd", "Omit LitOcc and Heap updates to reduce locking #NoAutoT", false, optionListPtr,
                              &opt_par_bve)
        ,
        ...
        , opt_bvaInpStepInc(_cat_bva, "cp3_bva_incInp", "increases of number of steps per inprocessing", 80000, IntRange(0, INT32_MAX), optionListPtr,
                            &opt_Abva)
        , opt_Abva_heap(_cat_bva, "cp3_Abva_heap",
                        "0: minimum heap, 1: maximum heap, 2: ratio pos/neg smaller+less, 3: ratio pos/neg smaller+greater, 4: ratio pos/neg "
                        "greater+less, 5: ratio pos/neg greater + greater, 6-9: same as 3-6, but inverse measure order",
                        1, IntRange(0, 9), optionListPtr, &opt_Abva)
        ,
        ...
        , opt_Ibva_maxRed(_cat_bva, "cp3_bva_Imax", "maximum reduction for one additional variable", INT32_MAX, IntRange(0, INT32_MAX), optionListPtr,
                          &opt_Ibva)
        , opt_Xbva_heap(_cat_bva, "cp3_Xbva_heap",
                        "0: minimum heap, 1: maximum heap, 2: ratio pos/neg smaller+less, 3: ratio pos/neg smaller+greater, 4: ratio pos/neg "
                        "greater+less, 5: ratio pos/neg greater + greater, 6-9: same as 3-6, but inverse measure order",
                        1, IntRange(0, 9), optionListPtr, &opt_Xbva)
        , opt_Ibva_heap(_cat_bva, "cp3_Ibva_heap",
                        "0: minimum heap, 1: maximum heap, 2: ratio pos/neg smaller+less, 3: ratio pos/neg smaller+greater, 4: ratio pos/neg "
                        "greater+less, 5: ratio pos/neg greater + greater, 6-9: same as 3-6, but inverse measure order",
                        1, IntRange(0, 9), optionListPtr, &opt_Ibva)
        ...
        // HBR
        //
        hbrLimit(_cat_hbr, "hbr-limit", "number of pairwise clause comparisons before interrupting HBR", 100000000, IntRange(0, INT32_MAX),
                 optionListPtr, &opt_hbr)
        ...
        , opt_hbr_addBinaries(_cat_hbr, "hbr-addBin", "when add binary clauses (0=always,1=1st iteration,2=never", 0, IntRange(0, 2), optionListPtr,
                              &opt_hbr)
        , opt_hbrInpStepInc(_cat_hbr, "hbr-incInp", "number of steps given to HBR for another inprocessign round", 10000, IntRange(0, INT32_MAX),
                            optionListPtr, &opt_hbr)
        ...
        , claStepMax(_cat_la, "la-claMax", "number of extension literals per step so that literals are removed randomly", 2, IntRange(1, INT32_MAX),
                     optionListPtr, &opt_la)
        , claIterations(_cat_la, "la-claIter", "number of extension literals per step so that literals are removed randomly", 1,
                        IntRange(1, INT32_MAX), optionListPtr, &opt_la)
        ,

        alaLimit(_cat_la, "ala-limit", "number of pairwise clause comparisons before interrupting LA", 100000000, IntRange(0, INT32_MAX),
                 optionListPtr, &opt_la_ala)
        , alaIterations(_cat_la, "la-alaIter", "number of extension literals per step so that literals are removed randomly", 1,
                        IntRange(1, INT32_MAX), optionListPtr, &opt_la_ala)
        ...
        , rate_Limit(_cat_rat, "rate-limit", "number of pairwise clause comparisons before interrupting RATE and up", 9000000000,
                     Int64Range(0, INT64_MAX), optionListPtr, &opt_rate)
        , ratm_Limit(_cat_rat, "ratm-limit", "number of pairwise clause comparisons before interrupting RATM and up", 9000000000,
                     Int64Range(0, INT64_MAX), optionListPtr, &opt_rate)
        ,
#ifndef NDEBUG
...
//
        ...
        , opt_eeGateBigFirst(_cat_ee, "cp3_BigThenGate", "detect binary equivalences before going for gates [should not be disabled!]", true,
                             optionListPtr, &opt_ee)
        ...
        , circ_BLOCKED(_cat_ee_hash, "cp3_extBlocked", "extract gates, that can be found by blocked clause addition", false, optionListPtr,
                       &opt_ee_level)
        , circ_AddBlocked(_cat_ee_hash, "cp3_addBlocked", "clauses that are used to extract blocked gates will be added eagerly (soundness)", false,
                          optionListPtr, &opt_ee_level)
...
// temporary Boolean flag to quickly enable debug output for the whole file
        ...
        // Fourier Motzkin
        //
        opt_fm_max_constraints(_cat_fm, "cp3_fm_maxConstraints", "number of constraints that are allows", 200000, IntRange(0, INT32_MAX),
                               optionListPtr, &opt_FM)
        , opt_fmLimit(_cat_fm, "cp3_fm_limit", "number of steps allowed for FM", 6000000, Int64Range(0, INT64_MAX), optionListPtr, &opt_FM)
        , opt_fmSearchLimit(_cat_fm, "cp3_fm_Slimit", "number of steps allowed for searching AMOs for FM", 12000000, Int64Range(0, INT64_MAX),
                            optionListPtr, &opt_FM)
        ...
        , opt_newAmo(_cat_fm, "cp3_fm_newAmo", "encode the newly produced AMOs (with pairwise encoding) 0=no,1=yes,2=try to avoid redundant clauses",
                     2, IntRange(0, 2), optionListPtr, &opt_FM)
        ...
        , opt_newAlo(_cat_fm, "cp3_fm_newAlo", "create clauses from deduced ALO constraints 0=no,1=from kept,2=keep all ", 2, IntRange(0, 2),
                     optionListPtr, &opt_FM)
        , opt_newAlk(_cat_fm, "cp3_fm_newAlk", "create clauses from deduced ALK constraints 0=no,1=from kept,2=keep all (possibly redundant!)", 2,
                     IntRange(0, 2), optionListPtr, &opt_FM)
        ...
        , opt_fm_printtrace(_cat_fm, "cp3_fm_printproof",
                            "print FM proof steps (0=off,1=ids,2=formula constraints,3=all constraints) use -no-cp3_fm_gc #NoAutoT", 0,
                            IntRange(0, 3), optionListPtr, &opt_FM)
        ...
        , pr_clsSize(_cat_pr, "pr-csize", "size of clauses that are considered for probing/vivification (propagation)", INT32_MAX,
                     IntRange(0, INT32_MAX), optionListPtr, &pr_probe)
        ...
        , pr_lcm(_cat_pr, "pr-lcm", "perform clause lcm vivification", false, optionListPtr, &opt_probe)
        , pr_lcm_vivi(_cat_pr, "pr-lcmC",
                      "Use vivifaction for clauses (1=plain vivi, 2=vivi+analyze, 3=vivi+unionanalyze, 4+reverse, >4 X-4, and-reverse)", 18,
                      IntRange(0, 24), optionListPtr, &pr_lcm)
        , pr_lcm_min_size(_cat_pr, "pr-lcmMinSize", "Apply LCM only to clauses with at least X literals", 3, IntRange(1, INT32_MAX), optionListPtr,
                          &pr_lcm)
        ...
        , pr_lcmLimit(_cat_pr, "pr-lcmLimit", "step limit for lcm-vivification", 5000000, IntRange(0, INT32_MAX), optionListPtr, &pr_lcm)
        , pr_lcm_iterations(_cat_pr, "pr-lcmIter", "number of iterations we should go over the clauses", 2, IntRange(0, INT32_MAX), optionListPtr,
                            &pr_lcm)
        ...
        , pr_opt_inpStepInc2(_cat_pr, "cp3_viv_inpInc", "increase for steps per inprocess call", 1000000, IntRange(0, INT32_MAX), optionListPtr,
                             &pr_vivi)
        , pr_keepLHBRs(_cat_pr, "pr-keepLHBR", "keep clauses that have been created during LHBR during probing/vivification (0=no,1=learnt)", 0,
                       IntRange(0, 1), optionListPtr, &opt_probe)
        ...
        , opt_lcm_vars(_cat, "cp3_lcm_vars", "variable limit to enable LCM-VIVIFICATION", 5000000, IntRange(0, INT32_MAX), optionListPtr, &pr_vivi)
        , opt_lcm_cls(_cat, "cp3_lcm_cls", "clause limit to enable LCM-VIVIFICATION", 10000000, IntRange(0, INT32_MAX), optionListPtr, &pr_vivi)
        , opt_lcm_lits(_cat, "cp3_lcm_lits", "total literal limit to enable LCM-VIVIFICATION", 15000000, IntRange(0, INT32_MAX), optionListPtr,
                       &pr_vivi)
        ,
#ifndef NDEBUG
...
//
        ...
        , opt_res3_steps(_cat_res, "cp3_res3_steps", "Number of resolution-attempts that are allowed per iteration", 1000000,
                         IntRange(0, INT32_MAX - 1), optionListPtr, &opt_ternResolve)
...
//
        ...
        ,
#endif
        opt_sls_ksat_flips(_cat_sls, "sls-ksat-flips", "how many flips should be performed, if k-sat is detected (-1 = infinite)", 20000000,
                           IntRange(-1, INT32_MAX), optionListPtr, &opt_sls)
        ...
        , opt_sub_allStrengthRes(_cat_sub, "all_strength_res",
                                 "Create all self-subsuming resolvents of clauses less equal given size (prob. slow & blowup, only seq)", 0,
                                 IntRange(0, INT32_MAX), optionListPtr, &opt_subsimp)
        ...
        , opt_sub_callIncrease(_cat_sub, "cp3_call_inc", "max. limit increase per process call (subsimp is frequently called from other techniques)",
                               200, IntRange(0, INT32_MAX), optionListPtr, &opt_subsimp)
        ...
        opt_sub_par_strength(_cat_sub, "cp3_par_strength", "par strengthening: 0 never, 1 heuristic, 2 always #NoAutoT", 1, IntRange(0, 2),
                             optionListPtr, &opt_subsimp)
        , opt_sub_lock_stats(_cat_sub, "cp3_lock_stats", "measure time waiting in spin locks #NoAutoT", false, optionListPtr, &opt_subsimp)
        , opt_sub_par_subs(_cat_sub, "cp3_par_subs", "par subsumption: 0 never, 1 heuristic, 2 always #NoAutoT", 1, IntRange(0, 2), optionListPtr,
                           &opt_subsimp)
        , opt_sub_par_subs_counts(_cat_sub, "par_subs_counts", "Updates of counts in par-subs 0: compare_xchange, 1: CRef-vector #NoAutoT", 1,
                                  IntRange(0, 1), optionListPtr, &opt_subsimp)
        , opt_sub_chunk_size(_cat_sub, "susi_chunk_size", "Size of Par SuSi Chunks #NoAutoT", 100000, IntRange(1, INT32_MAX), optionListPtr,
                             &opt_subsimp)
        , opt_sub_par_str_minCls(_cat_sub, "par_str_minCls", "number of clauses to start parallel strengthening #NoAutoT", 250000,
                                 IntRange(1, INT32_MAX), optionListPtr, &opt_subsimp)
        ,
        ...
        , sym_opt_hratio(_cat_sym, "sym-ratio", "only consider a variable if it appears close to the average of variable occurrences", 0.4,
                         DoubleRange(0, true, HUGE_VAL, true), optionListPtr, &opt_symm)
       ...
       //
        ...
        , opt_uhd_probe(_cat_uhd, "cp3_uhdProbe", "Approximate probing (bin cls) with stamp info (off,constant,linear,quadratic,exponential)", 0,
                        IntRange(0, 4), optionListPtr, &opt_unhide)
        , opt_uhd_fullProbe(_cat_uhd, "cp3_uhdPrSize", "Enable unhide probing for larger clauses, size <= given parameter", 2, IntRange(2, INT32_MAX),
                            optionListPtr, &opt_uhd_probe)
        ...
        , opt_xor_backdoor(_cat_xor, "xorBackdoor", "work on XOR backdoor, is size is smaller equal", 0, IntRange(0, INT32_MAX), optionListPtr,
                           &opt_xor)
        ...
        , opt_xor_setPolarity(_cat_xor, "xorSetPol", "set default polarities based on XOR elimination order and UP(-1=neg,1=pos)", 0, IntRange(-1, 1),
                              optionListPtr, &opt_xor)
        , opt_xor_addOnNewlyAdded(_cat_xor, "xorAddNew", "add simplified XORs to list of variables that have been added during add #NoAutoT", false,
                                  optionListPtr, &opt_xor)
        ,
